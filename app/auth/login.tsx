...
/** Premium kiritish maydoni — ikonka + flex input bitta chiroyli ramkada. */
function Field({
  icon, label, value, onChangeText, placeholder, keyboardType,
  secure, rightSlot,
}: {
  icon: IconName;
  label: string;
    ...
    <View style={styles.fieldWrap}>
      ...
      <View style={[styles.field, focused && styles.fieldFocused]}>
        <Ionicons
          name={icon}
          size={ms(19)}
          color={focused ? theme.colors.primary : theme.colors.faint}
        />
        <TextInput
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <View style={styles.segment}>
              <Pressable
                style={[styles.segBtn, !isRegister && styles.segBtnActive]}
                onPress={() => setMode('login')}
              >
                <Text
                  style={[styles.segText, !isRegister && styles.segTextActive]}
                  numberOfLines={1}
                >
                  Kirish
                </Text>
            ...
            <Button
              title={isRegister ? "Ro'yxatdan o'tish" : 'Kirish'}
              onPress={submit}
              loading={busy}
              disabled={!canSubmit}
          ...
          <View style={styles.trust}>
            {[
              ...
              <View key={b.label} style={styles.trustItem}>
                <Ionicons name={b.icon} size={ms(15)} color={theme.colors.brand} />
...
const styles = StyleSheet.create({
  ...
  logoWrap: {
    width: s(isSmall ? 64 : 76),
    height: s(isSmall ? 64 : 76),
    borderRadius: s(22),
  ...
  segBtnActive: { backgroundColor: theme.colors.card, ...theme.shadow.sm },
  segText: { fontSize: ms(13.5), fontWeight: '700', color: theme.colors.muted },
  segTextActive: { color: theme.colors.brand, fontWeight: '800' },

  greeting: { fontSize: ms(21), fontWeight: '800', color: theme.colors.text, letterSpacing: -0.3 },
  greetingSub: { fontSize: ms(13.5), color: theme.colors.muted, marginTop: s(5), lineHeight: ms(19) },

  fields: { gap: s(14), marginTop: s(22) },
  fieldWrap: { gap: s(7) },
  fieldLabel: { fontSize: ms(12.5), color: theme.colors.muted, fontWeight: '700', letterSpacing: 0.2, marginLeft: 2 },
  field: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: s(10),
  ...
  dividerText: { fontSize: ms(12), color: theme.colors.faint, fontWeight: '600' },

  switchBtn: { alignItems: 'center', paddingTop: s(16) },
  switchText: { fontSize: ms(14), color: theme.colors.muted, textAlign: 'center' },
  switchLink: { color: theme.colors.brand, fontWeight: '800' },
  ...
  trustText: { fontSize: ms(12), color: theme.colors.muted, fontWeight: '600' },
});
